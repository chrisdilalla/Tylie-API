...
function SendRequest(httpMethod, url, requestHeaders, requestBody, handleResponse) {
    ...
    var urlParts = encUrl.split("?");
    if (urlParts.length == 2) {
        queryParts = urlParts[1].split("&");
        for (var queryPartNum = 0; queryPartNum < queryParts.length; queryPartNum++) {
            var queryPart = queryParts[queryPartNum];
            var propWithValue = queryPart.split("=");
            propWithValue[0] = encodeURIComponent(propWithValue[0]);
            if (propWithValue.length == 2)
                propWithValue[1] = encodeURIComponent(propWithValue[1]);
            queryParts[queryPartNum] = propWithValue.join("=");
        }
        urlParts[1] = queryParts.join("&");
    }
    var resultUrl = urlParts.join("?");

    var httpRequest = new XMLHttpRequest();
    try {
        httpRequest.open(httpMethod, resultUrl, false);
    }
    catch (e) {
